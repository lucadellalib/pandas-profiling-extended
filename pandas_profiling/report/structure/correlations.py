...
def get_correlation_items(summary) -> Optional[Renderable]:
    ...
    for key, item in summary["correlations"].items():
        ...
        if len(description) > 0:
            ...
            correlation_tbl = Container(
                [correlation_matrix_diagram, desc], anchor_id=key, name=name, sequence_type="grid"
            )

            correlation_matrix_items.append(correlation_tbl)

            if predictivity_diagram is not None:
                predictivity_tbl = Container(
                    [predictivity_diagram, desc], anchor_id=key, name=name, sequence_type="grid"
                )
